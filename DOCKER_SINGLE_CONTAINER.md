...
docker build -t ghcr.io/<github-username>/alx-ecommerce:experimental .
docker login ghcr.io
docker push ghcr.io/<github-username>/alx-ecommerce:experimental
```
...
Troubleshooting
- If Postgres fails to start, inspect container logs:
```powershell
docker logs alx-ecommerce
```
- For interactive debugging, run a shell in the container:
```powershell
docker run -it --rm -p 8000:8000 -p 5432:5432 -e POSTGRES_PASSWORD=pass <image> /bin/bash
```

Cleaning up
```powershell
docker stop alx-ecommerce; docker rm alx-ecommerce; docker volume rm alx_pgdata
```

If you want, I can also produce a companion `docker-compose.yml` that uses the official `postgres` image and runs the app container separately — that is the recommended local/dev setup. Want that next?
